#!/usr/bin/env bash
# Resume/run the 12-arm adaptive SPRUCE WebArena Gmail sweep.
...
mkdir -p logs/adaptive_sweep reports/adaptive_sweep/spruce_60

TRACE_LOG="logs/adaptive_sweep/resume_adaptive_spruce_trace.log"
CONFIG="configs/webarena_gmail_adaptive_spruce_60.yaml"
TRIAL_NAME="webarena_gmail_adaptive_spruce_60"
REPORT_DIR="reports/adaptive_sweep/spruce_60"
STDOUT_LOG="logs/adaptive_sweep/adaptive_stdout.log"

echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] adaptive resume wrapper started pid=$$" >> "$TRACE_LOG"
...
log_dir = pathlib.Path("logs/adaptive_sweep")
segments = []
...
if [[ -n "$latest_log" ]]; then
    ...
    if [[ "$count" == "60" ]]; then
        ...
        export_run "$full_log"
        COMMIT_FINAL="${COMMIT_FINAL:-0}" bash scripts/finalize_adaptive_sweep.sh
        echo "adaptive SPRUCE resume/run complete"
        exit 0
    fi

    next_t=$((last_t + 1))
    bash scripts/finalize_adaptive_sweep.sh
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] resuming adaptive SPRUCE from $merged_so_far at t=$next_t" | tee -a "$STDOUT_LOG"
    cold-start-run --config "$CONFIG" --resume-from "$merged_so_far" --start-at "$next_t" 2>&1 | tee -a "$STDOUT_LOG"
else
    bash scripts/finalize_adaptive_sweep.sh
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] starting adaptive SPRUCE with $CONFIG" | tee -a "$STDOUT_LOG"
    cold-start-run --config "$CONFIG" 2>&1 | tee -a "$STDOUT_LOG"
fi

latest_log="$(latest_segment)"
if [[ -z "$latest_log" ]]; then
    echo "ERROR: no adaptive JSONL log found" >&2
    exit 1
fi
...
if [[ "$final_count" != "60" ]]; then
    bash scripts/finalize_adaptive_sweep.sh
    echo "ERROR: adaptive run has $final_count records after merge, expected 60: $full_log" >&2
    exit 1
fi
