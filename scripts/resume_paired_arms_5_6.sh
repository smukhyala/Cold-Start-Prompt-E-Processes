#!/usr/bin/env bash
# Resume/run arms 5 and 6 of the paired WebArena Gmail sweep:
#   5. balanced
#   6. overthinker
...
TRACE_LOG="logs/paired_sweep/resume_driver_arms_5_6_trace.log"
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] arms 5-6 resume wrapper started pid=$$" >> "$TRACE_LOG"
...
run_or_resume_arm \
    "balanced" \
    "configs/webarena_gmail_pair_balanced_60.yaml" \
    "webarena_gmail_pair_balanced_60"

run_or_resume_arm \
    "overthinker" \
    "configs/webarena_gmail_pair_overthinker_60.yaml" \
    "webarena_gmail_pair_overthinker_60"

echo "paired arms 5-6 resume/run complete"
