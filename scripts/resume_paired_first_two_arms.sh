#!/usr/bin/env bash
# Resume/run the first two arms of the paired WebArena Gmail sweep.
#
# This script is deliberately resume-aware:
#   - existing completed rows are merged before each resume
#   - the next task is last_t + 1
#   - final per-arm artifacts are built from a single *_FULL.jsonl file
...
TRACE_LOG="logs/paired_sweep/resume_driver_trace.log"
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] resume wrapper started pid=$$" >> "$TRACE_LOG"
...
echo "paired first-two resume/run complete"
