#!/usr/bin/env bash
# Resume/run the remaining paired WebArena Gmail sweep arms.
...
TRACE_LOG="logs/paired_sweep/resume_driver_remaining_trace.log"
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] remaining resume wrapper started pid=$$" >> "$TRACE_LOG"
...
export_arm() {
    ...
    bash scripts/finalize_paired_sweep.sh
}
...
run_or_resume_arm "overthinker" "configs/webarena_gmail_pair_overthinker_60.yaml" "webarena_gmail_pair_overthinker_60"
run_or_resume_arm "rapid" "configs/webarena_gmail_pair_rapid_60.yaml" "webarena_gmail_pair_rapid_60"
run_or_resume_arm "verifier" "configs/webarena_gmail_pair_verifier_60.yaml" "webarena_gmail_pair_verifier_60"
run_or_resume_arm "exploratory" "configs/webarena_gmail_pair_exploratory_60.yaml" "webarena_gmail_pair_exploratory_60"
run_or_resume_arm "algorithmic" "configs/webarena_gmail_pair_algorithmic_60.yaml" "webarena_gmail_pair_algorithmic_60"
run_or_resume_arm "junior_reactive" "configs/webarena_gmail_pair_junior_reactive_60.yaml" "webarena_gmail_pair_junior_reactive_60"
run_or_resume_arm "domain_expert" "configs/webarena_gmail_pair_domain_expert_60.yaml" "webarena_gmail_pair_domain_expert_60"

COMMIT_FINAL=1 bash scripts/finalize_paired_sweep.sh

if [[ -f /tmp/cold_start_full_sweep_caffeinate.pid ]]; then
    caffeinate_pid="$(cat /tmp/cold_start_full_sweep_caffeinate.pid 2>/dev/null || true)"
    if [[ -n "$caffeinate_pid" ]]; then
        kill "$caffeinate_pid" 2>/dev/null || true
    fi
    rm -f /tmp/cold_start_full_sweep_caffeinate.pid
fi
screen -S csp_caffeinate_full_sweep -X quit 2>/dev/null || true

echo "paired remaining resume/run complete"
