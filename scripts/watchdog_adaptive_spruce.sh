#!/usr/bin/env bash
# Watchdog for the unattended adaptive SPRUCE WebArena Gmail sweep.
...
SCREEN_NAME="${SCREEN_NAME:-csp_adaptive_spruce}"
WATCHDOG_PIDFILE="${WATCHDOG_PIDFILE:-/tmp/cold_start_adaptive_spruce_watchdog.pid}"
WATCHDOG_LOG="${WATCHDOG_LOG:-logs/adaptive_sweep/watchdog_adaptive_spruce.log}"
WRAPPER_STDOUT="${WRAPPER_STDOUT:-logs/adaptive_sweep/adaptive_stdout.log}"
WRAPPER_SCRIPT="${WRAPPER_SCRIPT:-scripts/resume_adaptive_spruce.sh}"
TRIAL_NAME="${TRIAL_NAME:-webarena_gmail_adaptive_spruce_60}"

mkdir -p logs/adaptive_sweep
echo $$ > "$WATCHDOG_PIDFILE"

log() { printf '[watchdog-adaptive %s] %s\n' "$(date -u +%H:%M:%SZ)" "$*" | tee -a "$WATCHDOG_LOG"; }
...
runner_alive() {
    pgrep -f 'cold-start-run --config configs/webarena_gmail_adaptive_spruce_60[.]yaml' >/dev/null 2>&1
}

wrapper_alive() {
    screen_alive || pgrep -f '[r]esume_adaptive_spruce[.]sh' >/dev/null 2>&1 || runner_alive
}
...
path = Path("docs/adaptive_sweep_status.md")
if not path.exists():
    raise SystemExit(1)
for line in path.read_text().splitlines():
    parts = [p.strip() for p in line.strip().strip("|").split("|")]
    if len(parts) == 5 and parts[0] == "adaptive_spruce":
        raise SystemExit(0 if parts[1] == "done" else 1)
raise SystemExit(1)
PY
}
...
paths = []
for p in pathlib.Path("logs/adaptive_sweep").glob(f"{trial_name}_trial0_*.jsonl"):
    name = p.name
...
restart_wrapper() {
    log "stopping screen=${SCREEN_NAME}, adaptive runner, and port 8001"
    screen -ls 2>/dev/null | awk -v name="$SCREEN_NAME" '$1 ~ "^[0-9]+[.]" name "$" {print $1}' | while read -r session; do
        screen -S "$session" -X quit 2>/dev/null || true
    done
    pgrep -f 'cold-start-run --config configs/webarena_gmail_adaptive_spruce_60[.]yaml' 2>/dev/null | while read -r pid; do
        [[ "$pid" == "$$" ]] && continue
...
while true; do
    if status_done; then
        log "adaptive run is done; exiting"
        exit 0
    fi
