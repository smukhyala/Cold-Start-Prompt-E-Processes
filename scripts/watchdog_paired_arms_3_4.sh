#!/usr/bin/env bash
# Watchdog for paired WebArena Gmail sweep arms 3 and 4.
...
SCREEN_NAME="${SCREEN_NAME:-csp_paired_arms_3_4}"
WATCHDOG_PIDFILE="${WATCHDOG_PIDFILE:-/tmp/cold_start_paired_arms_3_4_watchdog.pid}"
WATCHDOG_LOG="${WATCHDOG_LOG:-logs/paired_sweep/watchdog_paired_arms_3_4.log}"
WRAPPER_STDOUT="${WRAPPER_STDOUT:-logs/paired_sweep/resume_driver_arms_3_4_stdout.log}"
WRAPPER_SCRIPT="${WRAPPER_SCRIPT:-scripts/resume_paired_arms_3_4.sh}"

mkdir -p logs/paired_sweep
echo $$ > "$WATCHDOG_PIDFILE"

log() { printf '[watchdog-3-4 %s] %s\n' "$(date -u +%H:%M:%SZ)" "$*" | tee -a "$WATCHDOG_LOG"; }
...
runner_alive() {
    pgrep -f 'cold-start-run --config configs/webarena_gmail_pair_(cautious|explorer)_60[.]yaml' >/dev/null 2>&1
}

wrapper_alive() {
    screen_alive || pgrep -f '[r]esume_paired_arms_3_4[.]sh' >/dev/null 2>&1 || runner_alive
}
...
for line in path.read_text().splitlines():
    ...
    if len(parts) == 5 and parts[1] in {"cautious", "explorer"}:
        statuses[parts[1]] = parts[2]
raise SystemExit(0 if statuses.get("cautious") == "done" and statuses.get("explorer") == "done" else 1)
PY
}
...
explorer = candidates("webarena_gmail_pair_explorer_60")
cautious = candidates("webarena_gmail_pair_cautious_60")
if explorer:
    print(max(explorer, key=lambda p: p.stat().st_mtime))
elif cautious:
    print(max(cautious, key=lambda p: p.stat().st_mtime))
PY
}
...
restart_wrapper() {
    log "stopping screen=${SCREEN_NAME}, arms 3-4 runners, and port 8001"
    screen -ls 2>/dev/null | awk -v name="$SCREEN_NAME" '$1 ~ "^[0-9]+[.]" name "$" {print $1}' | while read -r session; do
        screen -S "$session" -X quit 2>/dev/null || true
    done
    pgrep -f 'cold-start-run --config configs/webarena_gmail_pair_(cautious|explorer)_60[.]yaml' 2>/dev/null | while read -r pid; do
        [[ "$pid" == "$$" ]] && continue
...
while true; do
    if status_done; then
        log "cautious and explorer are both done; exiting"
        exit 0
    fi
