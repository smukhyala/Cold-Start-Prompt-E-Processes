#!/usr/bin/env bash
# Watchdog for paired WebArena Gmail sweep arms 5 and 6.
...
SCREEN_NAME="${SCREEN_NAME:-csp_paired_arms_5_6}"
WATCHDOG_PIDFILE="${WATCHDOG_PIDFILE:-/tmp/cold_start_paired_arms_5_6_watchdog.pid}"
WATCHDOG_LOG="${WATCHDOG_LOG:-logs/paired_sweep/watchdog_paired_arms_5_6.log}"
WRAPPER_STDOUT="${WRAPPER_STDOUT:-logs/paired_sweep/resume_driver_arms_5_6_stdout.log}"
WRAPPER_SCRIPT="${WRAPPER_SCRIPT:-scripts/resume_paired_arms_5_6.sh}"

mkdir -p logs/paired_sweep
echo $$ > "$WATCHDOG_PIDFILE"

log() { printf '[watchdog-5-6 %s] %s\n' "$(date -u +%H:%M:%SZ)" "$*" | tee -a "$WATCHDOG_LOG"; }
...
runner_alive() {
    pgrep -f 'cold-start-run --config configs/webarena_gmail_pair_(balanced|overthinker)_60[.]yaml' >/dev/null 2>&1
}

wrapper_alive() {
    screen_alive || pgrep -f '[r]esume_paired_arms_5_6[.]sh' >/dev/null 2>&1 || runner_alive
}
...
for line in path.read_text().splitlines():
    ...
    if len(parts) == 5 and parts[1] in {"balanced", "overthinker"}:
        statuses[parts[1]] = parts[2]
raise SystemExit(0 if statuses.get("balanced") == "done" and statuses.get("overthinker") == "done" else 1)
PY
}
...
overthinker = candidates("webarena_gmail_pair_overthinker_60")
balanced = candidates("webarena_gmail_pair_balanced_60")
if overthinker:
    print(max(overthinker, key=lambda p: p.stat().st_mtime))
elif balanced:
    print(max(balanced, key=lambda p: p.stat().st_mtime))
PY
}
...
restart_wrapper() {
    log "stopping screen=${SCREEN_NAME}, arms 5-6 runners, and port 8001"
    screen -ls 2>/dev/null | awk -v name="$SCREEN_NAME" '$1 ~ "^[0-9]+[.]" name "$" {print $1}' | while read -r session; do
        screen -S "$session" -X quit 2>/dev/null || true
    done
    pgrep -f 'cold-start-run --config configs/webarena_gmail_pair_(balanced|overthinker)_60[.]yaml' 2>/dev/null | while read -r pid; do
        [[ "$pid" == "$$" ]] && continue
...
while true; do
    if status_done; then
        log "balanced and overthinker are both done; exiting"
        exit 0
    fi
    ...
    if [[ -z "$log_path" ]]; then
        ...
        if ! wrapper_alive || (( no_log_gap > NO_LOG_STALL_SECS )); then
            ...
            if wrapper_alive; then
                log "NO-LOG STALL: wrapper alive but no JSONL after ${no_log_gap}s"
            fi
            restart_wrapper
            restarts=$((restarts + 1))
            no_log_since="$(date +%s)"
