#!/usr/bin/env bash
# Watchdog for the unattended remaining-arm paired WebArena Gmail sweep.
...
SCREEN_NAME="${SCREEN_NAME:-csp_paired_remaining}"
WATCHDOG_PIDFILE="${WATCHDOG_PIDFILE:-/tmp/cold_start_paired_remaining_watchdog.pid}"
WATCHDOG_LOG="${WATCHDOG_LOG:-logs/paired_sweep/watchdog_paired_remaining.log}"
WRAPPER_STDOUT="${WRAPPER_STDOUT:-logs/paired_sweep/remaining_stdout.log}"
WRAPPER_SCRIPT="${WRAPPER_SCRIPT:-scripts/resume_paired_remaining_arms.sh}"

mkdir -p logs/paired_sweep
echo $$ > "$WATCHDOG_PIDFILE"

arms=(overthinker rapid verifier exploratory algorithmic junior_reactive domain_expert)
prefixes=(
    webarena_gmail_pair_overthinker_60
    webarena_gmail_pair_rapid_60
    webarena_gmail_pair_verifier_60
    webarena_gmail_pair_exploratory_60
    webarena_gmail_pair_algorithmic_60
    webarena_gmail_pair_junior_reactive_60
    webarena_gmail_pair_domain_expert_60
)

log() { printf '[watchdog-remaining %s] %s\n' "$(date -u +%H:%M:%SZ)" "$*" | tee -a "$WATCHDOG_LOG"; }
...
runner_alive() {
    pgrep -f 'cold-start-run --config configs/webarena_gmail_pair_(overthinker|rapid|verifier|exploratory|algorithmic|junior_reactive|domain_expert)_60[.]yaml' >/dev/null 2>&1
}

wrapper_alive() {
    screen_alive || pgrep -f '[r]esume_paired_remaining_arms[.]sh' >/dev/null 2>&1 || runner_alive
}
...
required = {"overthinker", "rapid", "verifier", "exploratory", "algorithmic", "junior_reactive", "domain_expert"}
...
for prefix in prefixes:
    all_paths.extend(candidates(prefix))
if all_paths:
    print(max(all_paths, key=lambda p: p.stat().st_mtime))
PY
}
...
restart_wrapper() {
    log "stopping screen=${SCREEN_NAME}, remaining runners, and port 8001"
    screen -ls 2>/dev/null | awk -v name="$SCREEN_NAME" '$1 ~ "^[0-9]+[.]" name "$" {print $1}' | while read -r session; do
        screen -S "$session" -X quit 2>/dev/null || true
    done
    pgrep -f 'cold-start-run --config configs/webarena_gmail_pair_(overthinker|rapid|verifier|exploratory|algorithmic|junior_reactive|domain_expert)_60[.]yaml' 2>/dev/null | while read -r pid; do
        [[ "$pid" == "$$" ]] && continue
...
while true; do
    if status_done; then
        log "remaining arms are done; exiting"
        exit 0
    fi
