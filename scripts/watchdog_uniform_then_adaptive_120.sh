#!/usr/bin/env bash
# Watchdog for sequential 120-task uniform then adaptive Gmail runs.
...
SCREEN_NAME="${SCREEN_NAME:-csp_uniform_then_adaptive_120}"
WATCHDOG_LOG="${WATCHDOG_LOG:-logs/comparison_sweeps_120/watchdog_uniform_then_adaptive_120.log}"
WRAPPER_STDOUT="${WRAPPER_STDOUT:-logs/comparison_sweeps_120/uniform_then_adaptive_120_stdout.log}"
WRAPPER_SCRIPT="${WRAPPER_SCRIPT:-scripts/resume_uniform_then_adaptive_120.sh}"

mkdir -p logs/comparison_sweeps_120

log() { printf '[watchdog-comparison-120 %s] %s\n' "$(date -u +%H:%M:%SZ)" "$*" | tee -a "$WATCHDOG_LOG"; }
...
runner_alive() {
    pgrep -f 'cold-start-run --config configs/webarena_gmail_(uniform_multiarm|adaptive_spruce)_120[.]yaml' >/dev/null 2>&1
}

wrapper_alive() {
    screen_alive || pgrep -f '[r]esume_uniform_then_adaptive_120[.]sh' >/dev/null 2>&1 || runner_alive
}
...
specs = [
    (pathlib.Path("logs/adaptive_sweep_120"), "webarena_gmail_adaptive_spruce_120"),
    (pathlib.Path("logs/uniform_multiarm_120"), "webarena_gmail_uniform_multiarm_120"),
]
paths = []
...
restart_wrapper() {
    log "stopping screen=${SCREEN_NAME}, 120-task runners, and port 8001"
    screen -ls 2>/dev/null | awk -v name="$SCREEN_NAME" '$1 ~ "^[0-9]+[.]" name "$" {print $1}' | while read -r session; do
        screen -S "$session" -X quit 2>/dev/null || true
    done
    pgrep -f 'cold-start-run --config configs/webarena_gmail_(uniform_multiarm|adaptive_spruce)_120[.]yaml' 2>/dev/null | while read -r pid; do
        [[ "$pid" == "$$" ]] && continue
        kill -9 "$pid" 2>/dev/null || true
    done
    pgrep -f '[r]esume_(uniform_multiarm_120|adaptive_spruce_120|uniform_then_adaptive_120)[.]sh' 2>/dev/null | while read -r pid; do
        [[ "$pid" == "$$" ]] && continue
...
while true; do
    if both_done; then
        log "120-task uniform and adaptive runs are done; exiting"
        exit 0
    fi
